...
def poleward_MSE_transport_model_vs_model(varnm,season,scale_ctl,scale_exp,pole):
    # data path
    ...
    if pole == "N":
        ...
        latbound2=nlat
        dtctl=file_ctl.variables[varnm][:,latbound1,:] #*scale_ctl
        dtexp=file_exp.variables[varnm][:,latbound1,:] #*scale_exp

	#latitude from degree to radians
        lat_rad=lat[latbound1]/180.*np.pi
	...
	#latitude from degree to radians
        lat_rad=lat[latbound2]*(-1)/180.*np.pi
                #length of the latitude * mean TVH
        tvh_tot=2.*np.pi*(rearth*np.cos(lat_rad))*\
        	np.mean(dtctl,axis=1)*(-1) #*-1 to get sourthward positive v
        #north cap area
        area_S=2.*np.pi*(rearth**2)*(1-np.sin(lat_rad))
        #area mean tvh
        tvh_mean_ctl=tvh_tot_ctl/area_S
        tvh_mean_exp=tvh_tot_exp/area_S
        tvh_mean_diff=tvh_mean_exp-tvh_mean_ctl
