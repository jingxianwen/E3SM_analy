...
def polar_stream_model_vs_model(season,scale_ctl,scale_exp,pole):
    # data path
    ...
    uctl=file_ctl.variables["U"][:,71,:,:]*scale_ctl
    uexp=file_exp.variables["U"][:,71,:,:]*scale_exp
    vctl=file_ctl.variables["V"][:,71,:,:]*scale_ctl
    vexp=file_exp.variables["V"][:,71,:,:]*scale_exp
    print(np.shape(uctl))
    print(np.shape(vctl))
    udif=uexp[:,:,:]-uctl[:,:,:]
    vdif=vexp[:,:,:]-vctl[:,:,:]
...
##    stats_ctl=get_area_mean_min_max(uctl[:,latbound1:latbound2,:],lat[latbound1:latbound2])
##    stats_exp=get_area_mean_min_max(uexp[:,latbound1:latbound2,:],lat[latbound1:latbound2])
##    stats_dif=get_area_mean_min_max(udif[:,latbound1:latbound2,:],lat[latbound1:latbound2])
...
##        p1 = ax.contourf(lon[:],lat[latbound1:latbound2],uplot[0,latbound1:latbound2,:],\
##                    transform=ccrs.PlateCarree(),\
##                    #norm=norm,\
##                    levels=cnlevels,\
##                    cmap=cmap,\
...
##            	    )
    ...
    fig.suptitle("surface wind "+season, x=0.5, y=0.96, fontdict=plotTitle)
    #save figure as file
    if os.environ["fig_save"]=="True":
        fname="d2_polar_stream_"+pole+"_"+season+"."+os.environ["fig_suffix"]
