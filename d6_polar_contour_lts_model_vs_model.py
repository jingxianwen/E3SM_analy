...
#lower troposphere stability (lts), defined as potential temperature at 700 hPa and surface.
def polar_contour_lts_model_vs_model(varnm,season,scale_ctl,scale_exp,pole):
    # data path
    varnm="LTS"
    ...
    dtctl_in=file_ctl.variables["T"][:,:,:,:]*scale_ctl
    dtexp_in=file_exp.variables["T"][:,:,:,:]*scale_exp
    #dtdif=dtexp[:,:,:,:]-dtctl[:,:,:,:]
    
    # calculate potential temperature for 700 hPa and surface.
    il_700=np.min(np.where(lev[:]>700))
    il_sfc=np.max(np.where(lev[:]>700))
    ptctl_700=dtctl_in[:,il_700,:,:]*((1000./lev[il_700])**0.286) # R/Cp=0.286
    ptexp_700=dtexp_in[:,il_700,:,:]*((1000./lev[il_700])**0.286) # R/Cp=0.286
    ptctl_sfc=dtctl_in[:,il_sfc,:,:]*((1000./lev[il_sfc])**0.286) # R/Cp=0.286
    ptexp_sfc=dtexp_in[:,il_sfc,:,:]*((1000./lev[il_sfc])**0.286) # R/Cp=0.286

    dtctl=ptctl_700-ptctl_sfc
    dtexp=ptexp_700-ptexp_sfc
    dtdif=dtexp[:,:,:]-dtctl[:,:,:]
    ...
    # make plot
    parameters=get_parameters("LTS",season)
